...
function woocommerce_wayforpay_init()
{
	...
	class WC_wayforpay extends WC_Payment_Gateway
	{
		...
		public function __construct(){
			$this->id = 'wayforpay';
			$this->method_title = __('WayForPay', 'wayforpay');
			$this->method_description = __('Card payments, Apple Pay and Google Pay.', 'wayforpay');
			...
			$this->serviceUrl = $this->settings['returnUrl'];// wtf?
			...
			add_filter( 'template_include', array(&$this, 'is_failed_payment'), 10, 1);
		}
		...
		public function is_failed_payment($template){
			...
			if(isset($_GET['order-received']) && isset($_GET['key']) && str_starts_with( $_GET['key'], self::ORDER_PREFIX ) ) {
				...
				if($paymentInfo === true) {
					wc_add_notice('Thank you for shopping with us. Your account has been charged and your transaction is successful.');
				}else{
					$order = new WC_Order(absint($_GET['order-received']));
					$error_message = __('A server error occurred while processing the payment. Error:', 'wayforpay')
					. $order->get_meta('reasonCode') . '(' . $order->get_meta('reason') . ")<br/>"
	                . '<a href="' .  $this->getUrlBySlug('checkout') . '">' . __('Return to checkout', 'wayforpay') . '</a>';
		...
		function init_form_fields(){
			$this->form_fields = array('enabled' => array('title' => __('Enable/Disable', 'woocommerce-wayforpay-payments'),
			                           ...
			                           'showlogo' => array('title' => __('Show Logo for classic checkout', 'woocommerce-wayforpay-payments'),
			                                               'type' => 'checkbox',
...
//			$woocommerce->cart->empty_cart();// do not need it if failed payment
		}
	...
	<script>
		 ...
		 **/
		public function generate_wayforpay_form($order_id){
		...
		protected function isPaymentValid($response){
			...
			if($response['transactionStatus'] == self::ORDER_APPROVED){
				...
				if(isset($_POST['reason']) && isset($_POST['reasonCode']) && empty($order->get_meta('reason'))) {
		 ...
		 */
		public function check_wayforpay_response(){
		 ...
		 * @return array
		 */
		public function wayforpay_get_pages($title = false, $indent = true){
			...
			foreach ($wp_pages as $page) {
				$prefix = '';
				if ($indent) {// show indented child pages?
				...
				$page_list[$page->ID] = $prefix . $page->post_title;// add to page list array array
			}
